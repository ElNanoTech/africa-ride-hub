...
-- Mirrored in TS by requiredZones() in src/lib/fleetControl.ts.
-- ---------------------------------------------------------------------
...
-- required zone set (was: always the full 11). Everything else identical
-- to the previous version (storage-object existence check included).
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- FC-A3 — fleet_control_approve: same derived required zone set (the
-- former require_all_photos-only gate always demanded the full 11 when
-- on, and nothing when off). Everything else identical to the previous
...
-- cycle_days setting, audit + driver notification.
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  IF v_existing IS NOT NULL THEN
    RETURN jsonb_build_object('created', false, 'control_id', v_existing);
  END IF;
  ...
  VALUES
    (v_vehicle.customer_id, p_vehicle, v_driver, v_rental.id, 'pending', v_cycle,
  ...
  RETURN jsonb_build_object('created', true, 'control_id', v_control.id);
END;
$fn$;
