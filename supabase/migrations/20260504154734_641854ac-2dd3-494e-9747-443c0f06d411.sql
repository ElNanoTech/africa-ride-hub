ALTER TABLE public.invoice ADD COLUMN IF NOT EXISTS rental_id uuid;
CREATE INDEX IF NOT EXISTS idx_invoice_rental ON public.invoice(rental_id) WHERE rental_id IS NOT NULL;
...
BEGIN
  ...
  -- Path A: payment was pre-linked to an invoice (admin issued via generate-invoice).
  -- Flip the linked invoice to paid in the same transaction.
  ...
  IF v_linked_invoice_id IS NOT NULL THEN
    ...
    IF FOUND THEN
      ...
      VALUES (v_linked_invoice_id, NEW.customer_id, 'paid', NULL, 'system',
              jsonb_build_object('payment_id', NEW.id, 'source', 'wave_webhook'));
    END IF;

    RETURN NEW;
  END IF;

  -- Path B: legacy auto-invoice on payment (no pre-existing link)
