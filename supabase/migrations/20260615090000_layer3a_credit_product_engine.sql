...
-- Product-driven credit catalog, applications, activation, and accounts.
-- ============================================================
...
CREATE UNIQUE INDEX IF NOT EXISTS uniq_product_versions_one_active
  ON public.product_versions(product_id)
  WHERE status = 'ACTIVE';

-- ---------- Assets, applications, snapshots ----------
...
CREATE TABLE IF NOT EXISTS public.credit_applications (
  ...
  eligibility_result text NOT NULL DEFAULT 'MANUAL_REVIEW' CHECK (eligibility_result IN (
    'NOT_ELIGIBLE','ALMOST_ELIGIBLE','ELIGIBLE_FOR_REVIEW','ELIGIBLE','MANUAL_REVIEW'
  )),
...
ALTER TABLE public.credit_applications
  ADD CONSTRAINT credit_applications_snapshot_fk
  FOREIGN KEY (snapshot_id) REFERENCES public.credit_snapshots(snapshot_id) ON DELETE SET NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS uniq_credit_asset_assignment_active
  ON public.credit_asset_assignments(asset_id)
  WHERE assignment_status = 'ACTIVE';

-- ---------- Decisions, agreements, fulfillment, activation, accounts ----------
...
ALTER TABLE public.credit_asset_assignments
  ADD CONSTRAINT credit_asset_assignments_account_fk
  FOREIGN KEY (credit_account_id) REFERENCES public.credit_accounts(credit_account_id) ON DELETE SET NULL;

CREATE UNIQUE INDEX IF NOT EXISTS uniq_credit_account_active_asset
  ON public.credit_accounts(asset_id)
  WHERE asset_id IS NOT NULL AND status IN ('ACTIVE','PAST_DUE','SUSPENDED');

-- ---------- Exposure, policy, audit ----------
...
ALTER TABLE public.invoice
  DROP CONSTRAINT IF EXISTS invoice_credit_obligation_type_check;

ALTER TABLE public.invoice
  ADD CONSTRAINT invoice_credit_obligation_type_check
  CHECK (
...
CREATE UNIQUE INDEX IF NOT EXISTS uniq_invoice_credit_idempotency
  ON public.invoice(customer_id, idempotency_key)
  WHERE idempotency_key IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_invoice_credit_application
  ON public.invoice(source_application_id)
...
ALTER TABLE public.invoice_audit
  ...
  CHECK (action IN (
    ...
    'fee_changed','regenerated_link','auto_generated','credit_obligation'
  ));
...
BEGIN
  FOREACH t IN ARRAY ARRAY[
    'vendors','credit_products','product_versions','financed_assets','credit_applications',
    'credit_asset_assignments','credit_decisions','fulfillment_records','activation_packages',
    'credit_accounts','credit_exposure_profiles','credit_policy_sets'
  ]
  LOOP
...
-- ---------- Immutable business-record guards ----------
...
AS $$
BEGIN
  RAISE EXCEPTION 'Immutable credit business record cannot be updated or deleted';
END;
$$;

DROP TRIGGER IF EXISTS trg_credit_snapshots_immutable_update ON public.credit_snapshots;
CREATE TRIGGER trg_credit_snapshots_immutable_update
  BEFORE UPDATE OR DELETE ON public.credit_snapshots
  FOR EACH ROW EXECUTE FUNCTION public.prevent_credit_immutable_change();

DROP TRIGGER IF EXISTS trg_credit_agreements_immutable_update ON public.credit_agreements;
CREATE TRIGGER trg_credit_agreements_immutable_update
  BEFORE UPDATE OR DELETE ON public.credit_agreements
  FOR EACH ROW EXECUTE FUNCTION public.prevent_credit_immutable_change();
...
DECLARE
  v_id uuid;
BEGIN
  INSERT INTO public.credit_audit_events (
    customer_id, actor_id, actor_type, action, entity_type, entity_id,
    before_state, after_state, metadata, idempotency_key
  )
  VALUES (
    p_customer_id,
    auth.uid(),
    CASE WHEN auth.uid() IS NULL THEN 'system' ELSE 'authenticated' END,
    p_action,
    p_entity_type,
    p_entity_id,
...
BEGIN
  FOREACH t IN ARRAY ARRAY[
    'vendors','credit_products','product_versions','financed_assets','credit_applications',
    'credit_snapshots','credit_asset_assignments','credit_decisions','credit_agreements',
    'fulfillment_records','activation_packages','credit_accounts','credit_exposure_profiles',
    'credit_policy_sets','credit_audit_events'
  ]
  LOOP
    ...
    EXECUTE format(
      'CREATE POLICY "credit platform owner all" ON public.%I FOR ALL TO authenticated USING (public.is_platform_owner()) WITH CHECK (public.is_platform_owner())',
      t
    );
    EXECUTE format(
      'CREATE POLICY "credit admins manage tenant" ON public.%I FOR ALL TO authenticated USING (public.has_credit_permission(''credit.admin'') AND customer_id = public.current_customer_id()) WITH CHECK (public.has_credit_permission(''credit.admin'') AND customer_id = public.current_customer_id())',
      t
    );
  END LOOP;
END;
...
CREATE POLICY "drivers read active credit products" ON public.credit_products
  FOR SELECT TO authenticated
  USING (status = 'ACTIVE' AND customer_id = public.current_driver_customer_id());

DROP POLICY IF EXISTS "drivers read active product versions" ON public.product_versions;
CREATE POLICY "drivers read active product versions" ON public.product_versions
  FOR SELECT TO authenticated
  USING (status = 'ACTIVE' AND customer_id = public.current_driver_customer_id());

DROP POLICY IF EXISTS "drivers read own credit applications" ON public.credit_applications;
CREATE POLICY "drivers read own credit applications" ON public.credit_applications
  FOR SELECT TO authenticated
...
CREATE POLICY "drivers read own credit snapshots" ON public.credit_snapshots
  FOR SELECT TO authenticated
  USING (EXISTS (
    SELECT 1 FROM public.credit_applications ca
    WHERE ca.application_id = credit_snapshots.application_id
...
CREATE POLICY "drivers read own credit decisions" ON public.credit_decisions
  FOR SELECT TO authenticated
  USING (EXISTS (
    SELECT 1 FROM public.credit_applications ca
    WHERE ca.application_id = credit_decisions.application_id
      AND ca.driver_id = public.current_driver_id()
  ));

DROP POLICY IF EXISTS "drivers read own activation packages" ON public.activation_packages;
CREATE POLICY "drivers read own activation packages" ON public.activation_packages
  FOR SELECT TO authenticated
  USING (EXISTS (
    SELECT 1 FROM public.credit_applications ca
    WHERE ca.application_id = activation_packages.application_id
      AND ca.driver_id = public.current_driver_id()
  ));

DROP POLICY IF EXISTS "drivers read own credit accounts" ON public.credit_accounts;
CREATE POLICY "drivers read own credit accounts" ON public.credit_accounts
  FOR SELECT TO authenticated
  USING (driver_id = public.current_driver_id());

DROP POLICY IF EXISTS "drivers read own fulfillment" ON public.fulfillment_records;
CREATE POLICY "drivers read own fulfillment" ON public.fulfillment_records
  FOR SELECT TO authenticated
  USING (EXISTS (
    SELECT 1 FROM public.credit_applications ca
    WHERE ca.application_id = fulfillment_records.application_id
      AND ca.driver_id = public.current_driver_id()
  ));

DROP POLICY IF EXISTS "drivers read assigned financed assets" ON public.financed_assets;
CREATE POLICY "drivers read assigned financed assets" ON public.financed_assets
  FOR SELECT TO authenticated
  USING (EXISTS (
    SELECT 1
    FROM public.credit_applications ca
    WHERE ca.requested_asset_id = financed_assets.asset_id
      AND ca.driver_id = public.current_driver_id()
  ) OR EXISTS (
    SELECT 1
    FROM public.credit_accounts cc
    WHERE cc.asset_id = financed_assets.asset_id
      AND cc.driver_id = public.current_driver_id()
  ));

DROP POLICY IF EXISTS "drivers read own exposure" ON public.credit_exposure_profiles;
CREATE POLICY "drivers read own exposure" ON public.credit_exposure_profiles
  FOR SELECT TO authenticated
  USING (driver_id = public.current_driver_id());

-- ---------- Exposure recomputation ----------
CREATE OR REPLACE FUNCTION public.credit_recompute_exposure(
  p_driver_id uuid,
  p_customer_id uuid,
  p_currency_code text DEFAULT 'XOF'
)
RETURNS public.credit_exposure_profiles
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  ...
  v_pending integer := 0;
  v_maximum integer := 0;
  v_profile public.credit_exposure_profiles;
BEGIN
  ...
  WHERE driver_id = p_driver_id
    ...
    AND principal_currency_code = p_currency_code
    AND status IN ('ACTIVE','PAST_DUE','SUSPENDED');

  SELECT COALESCE(SUM(fa.purchase_price), 0)::integer
    INTO v_pending
  FROM public.activation_packages ap
  JOIN public.credit_applications ca ON ca.application_id = ap.application_id
  LEFT JOIN public.financed_assets fa ON fa.asset_id = ca.requested_asset_id
  WHERE ca.driver_id = p_driver_id
    AND ca.customer_id = p_customer_id
    AND ca.status = 'APPROVED'
    AND ap.status IN ('PENDING','READY','BLOCKED')
    AND COALESCE(fa.purchase_price_currency_code, ca.down_payment_currency_code, p_currency_code) = p_currency_code
    AND NOT EXISTS (
      SELECT 1
      FROM public.credit_accounts cc
      WHERE cc.activation_package_id = ap.package_id
    );

  v_current := v_current + v_pending;

  SELECT COALESCE(maximum_exposure_limit, 0)
    INTO v_maximum
  ...
  INSERT INTO public.credit_exposure_profiles (
    driver_id, customer_id, maximum_exposure_limit, current_exposure,
    available_exposure, currency_code, last_calculated_at
  )
  VALUES (
    p_driver_id, p_customer_id, COALESCE(v_maximum, 0), v_current,
    GREATEST(COALESCE(v_maximum, 0) - v_current, 0), p_currency_code, now()
  )
  ON CONFLICT (customer_id, driver_id, currency_code) DO UPDATE
    SET current_exposure = EXCLUDED.current_exposure,
        available_exposure = EXCLUDED.available_exposure,
...
CREATE OR REPLACE FUNCTION public.submit_credit_application(
  p_product_id uuid,
  p_requested_asset_id uuid DEFAULT NULL,
  p_requested_terms_json jsonb DEFAULT '{}'::jsonb,
  p_kyc_reference_id uuid DEFAULT NULL,
  p_idempotency_key text DEFAULT NULL
)
RETURNS public.credit_applications
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  ...
  v_eligibility text;
  v_explanation text;
  v_down_payment_rule jsonb;
  v_purchase_amount integer := 0;
  v_currency text := 'XOF';
  v_down_payment integer := 0;
BEGIN
  ...
  FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'driver profile required' USING ERRCODE = '42501';
  END IF;

  SELECT * INTO v_application
  FROM public.credit_applications
  WHERE customer_id = v_driver.customer_id
  ...
  WHERE product_id = p_product_id
    AND customer_id = v_driver.customer_id
    AND status = 'ACTIVE';
  IF NOT FOUND THEN
    RAISE EXCEPTION 'active credit product not found' USING ERRCODE = 'P0002';
  END IF;
  ...
  LIMIT 1;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'active product version not found' USING ERRCODE = 'P0002';
  END IF;
  ...
  IF p_requested_asset_id IS NOT NULL THEN
    ...
    WHERE asset_id = p_requested_asset_id
      AND customer_id = v_driver.customer_id
    FOR UPDATE;
    IF NOT FOUND THEN
    ...
    IF v_asset.status NOT IN ('AVAILABLE','ASSIGNED') THEN
      RAISE EXCEPTION 'financed asset is not available for assignment' USING ERRCODE = '23514';
    END IF;
    v_purchase_amount := v_asset.purchase_price;
    v_currency := v_asset.purchase_price_currency_code;
  ELSE
  ...
  IF v_score IS NULL OR v_min_score IS NULL THEN
    ...
    v_explanation := 'Le score KIRA confirmé doit être disponible avant la revue crédit.';
  ELSE
    v_gap := GREATEST(v_min_score - v_score, 0);
    IF v_gap = 0 THEN
      v_eligibility := 'ELIGIBLE';
      v_explanation := COALESCE(v_rules->>'eligibility_explanation', 'Votre score KIRA confirmé atteint le minimum du produit.');
    ELSIF v_review_score IS NOT NULL AND v_score >= v_review_score THEN
      v_eligibility := 'ELIGIBLE_FOR_REVIEW';
      v_explanation := 'Votre score KIRA confirmé est proche du minimum. Revue manuelle requise.';
    ELSIF v_gap <= 75 THEN
      v_eligibility := 'ALMOST_ELIGIBLE';
      v_explanation := 'Non éligible - voir conditions. Il manque ' || v_gap::text || ' point(s).';
    ELSE
      v_eligibility := 'NOT_ELIGIBLE';
      v_explanation := 'Non éligible - voir conditions. Score minimum requis : ' || v_min_score::text || '.';
    END IF;
  END IF;
  ...
  INSERT INTO public.credit_applications (
    customer_id, driver_id, product_id, product_version_id, requested_asset_id,
    requested_terms_json, status, kyc_reference_id, submitted_at, expires_at,
    idempotency_key, eligibility_result, eligibility_explanation, score_snapshot,
    down_payment_amount, down_payment_currency_code, created_by, updated_by
  )
  VALUES (
    v_driver.customer_id, v_driver.id, v_product.product_id, v_version.version_id, p_requested_asset_id,
    COALESCE(p_requested_terms_json, '{}'::jsonb), 'SUBMITTED', p_kyc_reference_id, now(), now() + interval '30 days',
    p_idempotency_key, v_eligibility, v_explanation, v_score,
    v_down_payment, v_currency, auth.uid(), auth.uid()
  )
  RETURNING * INTO v_application;

  IF p_requested_asset_id IS NOT NULL THEN
    INSERT INTO public.credit_asset_assignments (
      customer_id, asset_id, application_id, assignment_status, idempotency_key, assigned_by
    )
    VALUES (
      v_driver.customer_id, p_requested_asset_id, v_application.application_id, 'ACTIVE',
      p_idempotency_key || ':asset', auth.uid()
    );

    UPDATE public.financed_assets
    SET status = 'ASSIGNED',
        fulfillment_status = CASE WHEN fulfillment_status = 'PENDING' THEN 'ASSIGNED' ELSE fulfillment_status END,
        updated_by = auth.uid()
    WHERE asset_id = p_requested_asset_id;
  END IF;
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'product_version_snapshot', jsonb_build_object(
      ...
      'eligibility_snapshot', jsonb_build_object(
        'result', v_eligibility,
        'explanation', v_explanation,
        'score', v_score,
        'min_score', v_min_score,
        'gap', v_gap,
        'source', 'driver_scores.current_score'
      ),
      'financial_snapshot', jsonb_build_object(
        'asset_price', v_purchase_amount,
        'down_payment_amount', v_down_payment,
        'currency_code', v_currency
      ),
      'asset_snapshot', CASE WHEN p_requested_asset_id IS NULL THEN NULL ELSE jsonb_build_object(
        ...
        'asset_condition', v_asset.asset_condition
      ) END,
      'kyc_reference_id', p_kyc_reference_id,
      'privacy_note', 'Snapshot keeps decision records and references; raw personal documents remain in controlled storage.'
    ),
    auth.uid()
  )
  ...
  PERFORM public.credit_log_event(
    ...
    jsonb_build_object('product_version_id', v_version.version_id, 'score_source', 'driver_scores.current_score'),
    p_idempotency_key
  );
...
BEGIN
  ...
  VALUES (
    ...
    v_invoice.id, p_idempotency_key, COALESCE(p_request_hash, encode(digest(p_application_id::text || p_idempotency_key, 'sha256'), 'hex')),
...
-- ---------- Launch seed: default tenant product catalog ----------
INSERT INTO public.vendors (vendor_id, customer_id, vendor_name, vendor_type, status, country, contact_information_json)
VALUES
...
INSERT INTO public.credit_products (
  product_id, customer_id, vendor_id, product_type, name, description, status,
  rules_json, eligibility_rules_json, down_payment_rules_json, asset_rules_json, activation_rules_json, visibility_rules_json
)
VALUES
  (
    '31000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000001',
    '30000000-0000-0000-0000-000000000001', 'CAR_OWNERSHIP', 'Vehicle Ownership Program',
    'Launch path for trusted drivers moving from rental readiness to vehicle ownership.', 'ACTIVE',
  ...
  (
    ...
    '30000000-0000-0000-0000-000000000002', 'MOTORCYCLE_FINANCING', 'Motorcycle Financing',
    'Configurable motorcycle financing foundation.', 'ACTIVE',
  ...
  (
    '31000000-0000-0000-0000-000000000003', '00000000-0000-0000-0000-000000000001',
    '30000000-0000-0000-0000-000000000003', 'PHONE_FINANCING', 'Phone Financing',
    'Configurable professional phone financing foundation.', 'ACTIVE',
  ...
  (
    '31000000-0000-0000-0000-000000000004', '00000000-0000-0000-0000-000000000001',
    NULL, 'TV_APPLIANCE_FINANCING', 'TV & Appliance Financing',
    'Configurable consumer goods financing foundation.', 'ACTIVE',
  ...
  (
    '31000000-0000-0000-0000-000000000005', '00000000-0000-0000-0000-000000000001',
    NULL, 'EQUIPMENT_FINANCING', 'Equipment Financing',
    'Configurable equipment financing foundation.', 'DRAFT',
...
ON CONFLICT (product_id) DO UPDATE
  SET name = EXCLUDED.name,
      ...
      down_payment_rules_json = EXCLUDED.down_payment_rules_json,
      asset_rules_json = EXCLUDED.asset_rules_json,
      activation_rules_json = EXCLUDED.activation_rules_json,
      visibility_rules_json = EXCLUDED.visibility_rules_json,
      updated_at = now();

INSERT INTO public.product_versions (
  version_id, customer_id, product_id, version_number, effective_from, effective_to, status, rules_snapshot_json
)
...
ON CONFLICT (version_id) DO UPDATE
  SET rules_snapshot_json = EXCLUDED.rules_snapshot_json,
      status = EXCLUDED.status,
      updated_at = now();

INSERT INTO public.financed_assets (
  asset_id, customer_id, asset_type, description, vendor_id,
  purchase_price, purchase_price_currency_code, residual_value, residual_value_currency_code,
  asset_condition, fulfillment_status, possession_status, status
)
VALUES
  (
    '33000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000001',
    'VEHICLE', 'Suzuki Dzire', '30000000-0000-0000-0000-000000000001',
    4000000, 'XOF', 1200000, 'XOF', 'NEW', 'PENDING', 'NOT_POSSESSED', 'AVAILABLE'
  ),
  (
    '33000000-0000-0000-0000-000000000002', '00000000-0000-0000-0000-000000000001',
    'MOTORCYCLE', 'Motorcycle launch asset', '30000000-0000-0000-0000-000000000002',
    1500000, 'XOF', 300000, 'XOF', 'NEW', 'PENDING', 'NOT_POSSESSED', 'AVAILABLE'
  ),
  (
    '33000000-0000-0000-0000-000000000003', '00000000-0000-0000-0000-000000000001',
    'PHONE', 'iPhone 13', '30000000-0000-0000-0000-000000000003',
    500000, 'XOF', 100000, 'XOF', 'NEW', 'PENDING', 'NOT_POSSESSED', 'AVAILABLE'
  )
ON CONFLICT (asset_id) DO UPDATE
  SET description = EXCLUDED.description,
      vendor_id = EXCLUDED.vendor_id,
      purchase_price = EXCLUDED.purchase_price,
      purchase_price_currency_code = EXCLUDED.purchase_price_currency_code,
      residual_value = EXCLUDED.residual_value,
      residual_value_currency_code = EXCLUDED.residual_value_currency_code,
      updated_at = now();

INSERT INTO public.credit_policy_sets (
  policy_id, customer_id, policy_name, policy_type, status, policy_json, effective_from
)
VALUES
  (
    '34000000-0000-0000-0000-000000000001',
    '00000000-0000-0000-0000-000000000001',
    'Layer 3A Exposure Foundation',
    'EXPOSURE_POLICY',
    'DRAFT',
    '{"evaluation_layer":"3B","note":"Stored in 3A, not enforced until policy engine layer."}'::jsonb,
