...
BEGIN
  ...
  -- Resolve admin_users.id from the auth user (FK target). If not found, skip the audit insert.
  SELECT id INTO v_admin_id FROM public.admin_users WHERE user_id = v_actor LIMIT 1;
