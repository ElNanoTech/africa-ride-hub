-- Layer 3A follow-up: platform owners have global RLS visibility, so
-- state-changing credit RPCs must load the target application with
-- platform-or-tenant scope before doing tenant-bound idempotency checks.
